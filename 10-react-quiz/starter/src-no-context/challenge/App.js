...
function reducer(state, action) {
    switch (action.type) {
        ...
        case 'withdraw':
            return {
                ...state,
                balance: state.balance >= action.payload ? state.balance - 50 : state.balance
            }

        case 'loan':
            return {
                ...state,
                balance: !state.isLoanActive ? state.balance + LOAN_AMOUNT : state.balance,
                loan: !state.isLoanActive ? LOAN_AMOUNT : state.loan,
                isLoanActive: true,
            }
        ...
        case 'payLoan':
            return {
                ...
                loan: state.isLoanActive ? 0 : state.loan,
                isLoanActive: false,
            };
...
export default function App() {
    ...
    return (
        <div style={{ fontFamily: "sans-serif", textAlign: "center" }}>
            ...
            <p>
                <button onClick={() => dispatch({ type: 'deposit150' })} disabled={!isActive}>
                    Deposit 150
                </button>
            </p>
            <p>
